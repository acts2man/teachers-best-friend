-- Admin storage figures were read from public.uploads, which the application
-- stopped writing to. The app records every upload in public.teacher_uploads,
-- so the dashboard reported a stale subset: 12 files and 18 MB against an
-- actual 28 files and 52 MB.
--
-- The two tables are not additive. Every row in the legacy table points at the
-- same object_path as a row in teacher_uploads, so summing both would double
-- count those files. teacher_uploads matches the bucket exactly (28 rows, 28
-- objects), so it is the one to count.
...
create or replace view public.admin_accounts as
 SELECT u.id AS teacher_id,
    ...
    ( SELECT count(*) AS count
           FROM teacher_uploads up
          WHERE ((up.owner_id = u.id) AND (up.purged_at IS NULL))) AS uploads,
    ( SELECT COALESCE(sum(up.size), (0)::bigint) AS "coalesce"
           FROM teacher_uploads up
