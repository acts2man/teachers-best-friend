...
64 of the 73 are byte-identical to what was applied. The other nine were
...
Those two numbers go stale every time a migration is applied, so do not trust
them — re-measure. `select version || '_' || name, md5(statements[1] || E'\n')
from supabase_migrations.schema_migrations` gives the MD5 of each file as it
should be on disk; compare it against `md5sum`. Every applied migration must
have a file, and the count of files that differ must be those same nine.
