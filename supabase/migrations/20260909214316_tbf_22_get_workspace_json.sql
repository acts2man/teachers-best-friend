-- ===============================================================
-- READ FACADE
-- Assembles the exact workspace shape the client already consumes,
-- from the relational tables. Replaces `select data from
-- teacher_workspaces` on the GET path with one RPC call.
-- Settings and activeClassId still live on teacher_workspaces —
-- they are the only things left in that row.
-- ===============================================================
...
with
cls as (
  select c.*, c.legacy_id as cid from public.classes c
   where c.teacher_id = p_teacher and c.archived_at is null),
stu as (
  select s.*, coalesce(cl.legacy_id, s.legacy_class_id) as class_legacy
...
asm as (
  select a.*, cl.legacy_id as class_legacy
...
select jsonb_build_object(
  'classes', coalesce((
     select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
       'id', cid, 'name', name, 'grade', grade, 'framework', framework,
       'demo', case when is_demo then true end)) order by created_at)
     from cls), '[]'::jsonb),

  'students', coalesce((
     select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
       'id', s.legacy_id, 'classId', s.class_legacy,
       'name', s.display_label, 'fullName', s.full_name,
       'color', s.color, 'notes', s.notes,
       'evidence', coalesce((
          select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
            'id', e.legacy_id, 'standard', e.standard_code, 'score', e.score,
            'date', e.recorded_on, 'source', e.source, 'assessmentId', ea.legacy_id))
            order by e.recorded_on, e.created_at)
  ...
  'assessments', coalesce((
     select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
       'id', a.legacy_id, 'classId', a.class_legacy,
       'classIds', to_jsonb(a.class_ids),
       ...
       'targetStandards', to_jsonb(a.target_standards),
       'passage', a.passage, 'answerKeyVerified', a.answer_key_verified,
       ...
       'questions', coalesce((
          select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
            'id', q.legacy_id, 'number', q.number, 'text', q.text, 'passage', q.passage,
            'answer', q.answer, 'standard', q.standard_code, 'secondary', q.secondary_standard_code,
  ...
  'lessons', coalesce((
     select jsonb_agg(l.body order by l.created_at)
  ...
  'groups', coalesce((
     select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
       ...
       'studentIds', coalesce((
          select jsonb_agg(st.legacy_id) from public.student_group_members m
          join public.students st on st.id = m.student_id where m.group_id = g.id), '[]'::jsonb)))
       order by g.created_at)
     from public.student_groups g left join public.classes gc on gc.id = g.class_id
     where g.teacher_id = p_teacher), '[]'::jsonb),

  'customStandards', coalesce((
     select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
       'code', s.code, 'title', s.short_label, 'subject', s.subject, 'grade', s.grade,
       'domain', s.domain, 'cluster', s.cluster, 'wording', s.description,
       'framework', s.framework)) order by s.created_at)
     from public.standards s where s.teacher_id = p_teacher), '[]'::jsonb),
...
revoke execute on function public.get_workspace_json(uuid) from public, anon, authenticated;
